...
from .lib.model import get_face_alignment_net, get_cls_net
...
def get_model_by_name(model_name, root_models_path='hrnetv2_models', prefix='HR18-', model_type='landmarks', device='cuda'):
    ...
    checkpoint_path = f'models/face_landmark/{root_models_path}/{prefix}{model_name}.pth'
    config_path = f'models/face_landmark/{root_models_path}/{prefix}{model_name}.yaml'
    ...
    model.load_state_dict(torch.load(checkpoint_path, map_location="cpu"))   # load model to cpu by default
    model.eval()
...
def get_lmks_by_img(model, img, output_size=(256, 256), dev="cuda", rot=0):
