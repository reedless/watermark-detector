...
def get_all_landmarks_from_net(landmarks):
...
def get_lip_landmarks_from_net(landmarks):
    """
    Return landmarks of top lip and bottom lip
    """
    top_lip = np.concatenate((landmarks[76:82+1], landmarks[88:92+1][::-1]), axis=0)
    bottom_lip = np.concatenate((landmarks[82:87+1], np.array([landmarks[76]]), np.array([landmarks[88]]), landmarks[92:95+1][::-1]), axis=0)
    return np.array([top_lip, bottom_lip])
...
def show_img_landmarks(img, figsize=(10,8)):
    """
    Plot landmarks on image
    :param img: source image
    :param lmks: landmarks to chow
