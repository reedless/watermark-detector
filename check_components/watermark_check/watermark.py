...
def check_watermark(cfg, input_im, face_foreground_background_res, background_check_result, face_highlight_res):
    ...
    for i in range(len(output['instances'].pred_masks)):
        ...
        # if significant_overlap(instance, face_foreground_background_res[:,:,0].reshape(-1) < 250, 0.8):
        ...
        if significant_overlap(instance, face_highlight_res["res_highlight_mask"].reshape(-1), 0.5):
            removed_specular_watermarks += 1
            continue
    ...
    remarks = (f'{remarks} '
               f'{removed_background_watermarks} watermark(s) in the background removed. ' 
               f'{removed_specular_watermarks} watermark(s) removed due to overlap with specular reflection. ')
...
if __name__ == '__main__':
    ...
    # im = cv2.imread('dataset/benchmarkv2/watermarked_stamp2.jpg')
    # output = predictor(im)
    ...
    for i in tqdm(os.listdir('dataset/benchmarkv3')):
        if i[-15:] == 'Zone.Identifier':
            continue
        im = cv2.imread(os.path.join('./dataset/benchmarkv3', i))
        ...
        if not os.path.exists('./output/benchmarkv3/raw'):
            os.mkdir('./output/benchmarkv3/raw')
        plt.savefig(f'./output/benchmarkv3/raw/{i}')
        plt.close()
